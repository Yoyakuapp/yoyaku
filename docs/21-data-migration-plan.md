...
`20260712140000_add_service_menus` は `ServiceMenu` を作成し、既存の30/60/90/120分メニューを初期Storeへseedします。BookingとBookingPaymentAttemptには互換性維持のためnullableの`serviceMenuId`を追加します。

このMigrationはSQLレビュー後にMasaがNeonへ適用します。`prisma migrate reset` と `prisma db push` は使用しません。
